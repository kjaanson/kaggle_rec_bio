...
@retry(tries=3)
def get_input(experiment, plate, well, site, channel, train=True, path='./input/recbio'):
...
class ImgGen(Sequence):
    ...
    def __getitem__(self, i):
        ...
        x_s1_c1 = [np.array(self.preprocess(get_input(e, p, w, site=1, channel=1, path=self.path)))/255 for e, p, w in batch_x.values.tolist()]
        x_s1_c2 = [np.array(self.preprocess(get_input(e, p, w, site=1, channel=2, path=self.path)))/255 for e, p, w in batch_x.values.tolist()]
        x_s1_c3 = [np.array(self.preprocess(get_input(e, p, w, site=1, channel=3, path=self.path)))/255 for e, p, w in batch_x.values.tolist()]
        
        x_s1_c4 = [np.array(self.preprocess(get_input(e, p, w, site=1, channel=4, path=self.path)))/255 for e, p, w in batch_x.values.tolist()]
        x_s1_c5 = [np.array(self.preprocess(get_input(e, p, w, site=1, channel=5, path=self.path)))/255 for e, p, w in batch_x.values.tolist()]
        x_s1_c6 = [np.array(self.preprocess(get_input(e, p, w, site=1, channel=6, path=self.path)))/255 for e, p, w in batch_x.values.tolist()]
