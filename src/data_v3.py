...
class ImgGen(Sequence):
    ...
    def __init__(self,
        ...
        if not all([col in metadata_df.columns for col in ['experiment', 'plate', 'well_name']]):
            raise ValueError('metadata_df must have columns experiment, plate and well_name')

        self.metadata = metadata_df

        self.file_path_metadata = pd.DataFrame(self.img_file_list, columns=['file_path'])
        self.file_path_metadata[["experiment","plate","well_name","well_position"]] = list(self.file_path_metadata.file_path.apply(parse_path_for_metadata))

        self.file_path_metadata = self.file_path_metadata.merge(self.metadata, on=['experiment', 'plate', 'well_name'], how='left')

        # throw error if file path metadata has NaN values
        if self.file_path_metadata.isnull().values.any():
            raise ValueError('file_path_metadata must have no NaN values, maybe there is missing metadata?')
